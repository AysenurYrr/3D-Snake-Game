...
def check_collision():
    ...
    head = snake[-1]
    if ( speed > 60 and (head == gold_apple or head == diamond_apple or head == red_apple or head == stone_apple)):
...
def place_red_apple():
    ...
    while True:
        ...
        if new_red_apple not in snake and new_red_apple not in bomb_apples:
...
def place_bomb_apples():
    ...
    while len(bomb_apples) < 4:
        ...
        if new_bomb_apple not in snake and new_bomb_apple != red_apple and new_bomb_apple not in bomb_apples:
...
def restart_game():
    global snake, snake_dir, game_over, red_apple, diamond_apple, stone_apple, gold_apple, main_window, game_over_window
    snake = [(5, 5)]
    snake_dir = (1, 0)
    game_over = False
    red_apple = (random.randint(0, map_size-1), random.randint(0, map_size-1))
